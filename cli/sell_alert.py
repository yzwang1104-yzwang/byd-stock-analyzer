"""卖出提醒 — 低点买入持仓在价格回弹至历史最高 70%-80% 时告警。

触发条件（必须同时满足）：
  1. 低点买入：持仓均价 ≤ 历史最低价 × 1.15
  2. 目标区间：high_all × 70% ≤ 当前实时价 ≤ high_all × 80%
...
SELL_LOWER = 0.95          # 默认目标区间下边界：历史最高 × 95%
SELL_UPPER = 0.98          # 默认目标区间上边界：历史最高 × 98%

# 单只股票自定义卖出比例（覆盖默认值）
CUSTOM_SELL: dict[str, tuple[float, float]] = {
    # "000690": (0.90, 0.95),  # 示例：单只股票自定义
}
