"""持仓分析表 — 14 列固化格式（2026-07-28 定版，2026-08-21 盈亏列移至末位）。

列: 代码/名称/股数/均价/现价/涨跌/市值/盈%/最高/95%目标/需涨%/目标-均价/潜在利润/盈亏
...
def main() -> None:
    ...
    hdr = (f"{'代码':<8} {'名称':<8} {'股数':>5} {'均价':>7} {'现价':>7} {'涨跌':>6} "
           f"{'市值':>9} {'盈%':>7} {'最高':>8} {'95%目标':>8} {'需涨%':>7} "
           f"{'目标-均价':>9} {'潜在利润':>9} {'盈亏':>8}")
    print(hdr)
    print("-" * 118)
    ...
    for r in rows:
        ...
        print(f"{r['code']:<8} {r['name']:<8} {r['shares']:>5} {r['avg']:>7.2f} "
              ...
              f"{r['pnl_pct']:>+6.1f}% {fmt(r['high']):>8} "
              f"{fmt(r['target']):>8} {fmt(r['to_target'], '%'):>7} "
              f"{fmt(r['gap']):>9} {fmt(r['potential']):>9} {r['pnl']:>+8.0f}")
