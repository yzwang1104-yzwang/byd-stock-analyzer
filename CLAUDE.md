...
## 三十四、2026-08-19 ~ 2026-08-20 会话记录

### 时间线

| 日期 | 事件 |
|------|------|
| 08-19 | 高分红扫描（cli/high_dividend.py 新建）：22 只候选，泸州老窖 12.27% 第一；吉比特补查 5.09% 排第 10 |
| 08-19 | 个股历史高低点连查 13 只（002555/600036/301601/688639/300822/688157/600690/600406/600585/600744/601985/600909/601727）|
| 08-19 | 买入判断：三七互娱/招行都"再等等"（等回调）|
| 08-20 | **记录买入 600845 宝信软件 100股@17.7902**（含费用 -5.02）|
| 08-20 | **记录卖出 603395 红四方 100股@30.00**，净盈利 +883.94（+41.9%）|
| 08-20 | 卖出分析实时化：sell_analysis.py 默认实时拉取（腾讯 fqkline + qt），失败降级缓存 |
| 08-20 | 实时卖出分析：强烈卖出 5 只（三维化学 89 飞刀/华新精科 85 飞刀/天马科技 77 飞刀/万憬 75/丰倍 75），\*ST三房 65 |
| 08-20 | 已卖出复盘（cli/_sold_analysis.py）：3 笔全胜 +1512.63 元 |
| 08-20 | **主板盈利股筛选**：46 只四因子结果 → 剔除创业板/科创板/北交所 → 剔除 3 只亏损股 → **21 只主板盈利股** |
| 08-20 | 银证转账咨询：双向免费，卖出资金 T+1 可转出 |

### 关键技术点

| 技术点 | 说明 |
|------|------|
| 腾讯 fqkline 上限 | count=4000 报 param error；2000 可用但实际返回约 640 条（≈2.5 年窗口）|
| qt 实时价提取 | 正则 `~([\d.]+)~` 会误匹配股票代码 → 先 `r'="([^"]*)"'` 提取值再 split("~") 取 parts[3] |
| 亏损股判定 | 实时负 PE 验证：通威 -5.87 / 沙河 -16.02 / 隆基 -13.58，全部剔除 |
| fqkline 防御 | data 为 list（param error 响应）时 isinstance(node, dict) 防御 |

### 21 只主板盈利股（四因子：5-20元 + 距低≤3元 + 距高≥10元 + 估值低 + 趋势非DN）

按距高降序：口子窖(31.71元 UP)、上海电影(23.52)、宝信软件🔒(21.80)、同庆楼(21.43 UP)、长城汽车(17.03 UP)、江铃汽车(16.87)、洽洽食品(16.30)、北新建材(16.16)、人民网(15.55 UP)、森麒麟(15.11 UP)、天坛生物🔒(14.63 UP)、锦江酒店(14.34)、中科软(13.74)、国盛证券(13.45)、老白干酒(13.12 UP)、首创证券(12.90 UP)、久远银海(12.38 UP)、广博股份(12.24 UP)、中航重机(11.64)、上汽集团🔒(10.81 UP)、索菲亚(10.67 UP)

PE 最低：索菲亚 9.3 / 北新建材 11.0 / 上汽 11.5 / 江铃 11.8；注意上海电影(121)/国盛证券(103) 市盈率绝对值偏高

### 持仓变化

| 操作 | 代码 | 名称 | 结果 |
|------|------|------|------|
| 买入 | 600845 | 宝信软件 100股@17.7902 | 新持仓 |
| 卖出 | 603395 | 红四方 100股@30.00 | 净盈利 +883.94（+41.9%）|

### 已清仓复盘（3 笔全胜）

红四方 +883.94 / 江苏华辰 +585.86 / 国电电力 +42.83 = **+1512.63 元，胜率 100%**，模式 = 低点买入高点区域卖出

### 教训

1. **fqkline 窗口只有 2.5 年（640 条）**。更早历史需其他数据源，筛选"历史最高"受此限制。
2. **"要挣钱的"筛选必须剔除负 PE**。通威/沙河/隆基都贴着历史低位、估值分位显示"?"，实时 PE 全负——亏损股再便宜也是陷阱。
3. **银证转账免费，卖出资金 T+1 到账**。不构成任何费用。
4. **高股息率 ≠ 高每股派息**。吉比特每股派 19.60 元全场第二，但股价 384.79 拉低股息率至 5.09%。
...
| 时间 | 事件 |
|------|------|
| 08-21 | 主板盈利股收尾：PE 显示"?"的股票实时验证全部负 PE（航锦科技 -67.49 等）→ 剔除 → **21 只主板盈利股名单最终确定** |
| 08-21 | 603395 卖出费用明细：佣金 5 + 印花税 1.50 + 过户费 0.03 = **6.53 元** |
| 08-21 | 佣金优化策略（5 元红线）：单笔 < 2 万费率惩罚严重，结论 = 单笔 ≥ 2 万 + 低频 + 大波段 |
| 08-21 | 30×2 万方案评估：60 万费用最优，但 30 只 = 人肉指数基金，ETF（免印花税）更优 |
| 08-21 | ETF 教学：15 个主流 ETF 实时验证（510300 沪深300 / 510500 中证500 / 510050 上证50 / 159915 创业板 / 588000 科创50 / 510880 红利 / 512480 半导体 / 512170 医疗 / 513100 纳指 / 518880 黄金 / 511010 国债 / 511260 十年国债 / 159919 沪深300嘉实 / 512000 券商 / 513180 恒生科技）|
| 08-21 | 个股高低点连查：603589 口子窖、000818、600776、601136、000651、000895、600585+601006 |
| 08-21 | 603395 红四方暴涨定性：公司公告无未披露利好、2025 净利 -45%、换手 43.72% → **纯游资炒作**，30 元卖出是纪律性止盈 |
| 08-21 | 用户质疑白酒推荐：「为什么你觉得白酒会升职」→ 诚实回答：四因子选的是赔率结构（下 9% 上 60%）不是行业判断，白酒有真实逆风（年轻消费下降、库存高企），跌透 ≠ 马上涨 |
| 08-21 | 今日买入预测：buy_scan_fast 扫描 5606 只 → TOP40 过滤主板 → TOP10（一汽解放 97 / 国联民生 95 / 上汽 94 / 中交设计 94 / 首创证券 94 / 长城汽车 94 / 江铃 93 / 久远银海 93 / 人民网 93 / 中航重机 92）；双重确认 8 只（长城/上汽/首创/江铃/久远/人民网/中航重机/索菲亚）|
| 08-21 | 高分红重扫：**格力 12.07% 登顶**（特别分红叠加）、泸州老窖 12.04%、吉比特 4.98% 第 11 |
| 08-21 | **持仓表格式固定**：用户多轮迭代后确认「以后要持仓分析就用现在这个表格样式」→ 盈亏列移至末位（14 列定版）|
| 08-21 | **记录卖出 000983 山西焦煤 100股@7.05**：净盈利 +69.64（+11.1%，持仓 32 天）→ 已清仓 4 笔全胜 +1,582.27 |

### 关键技术点

| 技术点 | 说明 |
|------|------|
| 佣金 5 元红线 | 佣金 = max(成交额 × 万2.5, 5 元)；来回成本：1000 元 → 1.05%、3000 元 → 0.385%、2 万元 → 0.102%；卖出印花税 0.05% 必交 |
| 场内 ETF 优势 | 免印花税、免过户费、100 份起、股票账户直接买卖；场外联接基金费率更高 |
| 持仓表 14 列定版 | 代码/名称/股数/均价/现价/涨跌/市值/盈%/最高/95%目标/需涨%/目标-均价/潜在利润/**盈亏**（盈亏在末位），潜在利润降序 |

### 教训

1. **用户不喜欢被提问，偏好直接给出合理解读。** 「持仓利润列」口径我用 AskUserQuestion 询问被拒，用户直接打断澄清「是我持有的股票已经盈利多少钱的总数」。有持仓上下文时应先自行消解歧义（同 08-18 利润提醒教训）。
2. **「推荐」与「行业看好」必须分开。** 四因子选出白酒是赔率结构（下有支撑、上有空间），不代表行业拐点判断。用户问「为什么觉得白酒会升值」时，诚实说明机制与局限，不硬撑。
3. **游资炒作定性依赖公司公告。** 红四方暴涨核查路径：异常波动公告（无未披露利好）+ 业绩预告（净利 -45%）+ 换手率 43.72% → 结论可信。

### 当前状态

```
持仓: 20只活跃 | 已清仓 4笔全胜 +1,582.27（红四方+883.94/江苏华辰+585.86/山西焦煤+69.64/国电电力+42.83）
今日买入 TOP10: 一汽解放97/国联民生95/上汽94/中交设计94/首创证券94/长城汽车94/江铃93/久远银海93/人民网93/中航重机92
高分红 TOP3: 格力 12.07% / 泸州老窖 12.04% / 吉比特 4.98%
持仓表: 14列定版(盈亏末位) | memory已固化
```
