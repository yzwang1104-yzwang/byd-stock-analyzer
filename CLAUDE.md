...
| 脚本 | 用途 | 趋势 |
|------|------|:--:|
| `cli/_scan_realtime.py` | 全量实时买入提醒 | 39/40 DN |
| `cli/_scan_trend_up.py` | 趋势过滤（仅UP/--） | 34/40 UP |
| `cli/sell_alert.py` | 卖出提醒 | — |
...
- **表格格式:** 持仓表格固定12列（代码/名称/股数/均价/现价/涨跌/市值/盈亏/盈亏%/最高/95%目标/需涨%），不再增减
- **价格来源:** 优先腾讯实时行情(parts[3])，收盘后用K线收盘价
- **名称来源:** 腾讯实时API查询，不手写KNOWN字典
- **回答简洁:** 40行列表直接展示，不压缩成"..."
- **买入提醒/趋势扫描/距低扫描:** 用 `cli/_scan_realtime.py` 和 `cli/_scan_trend_up.py`，不用旧版 buy_alert.py
