...
3. 无 V 型反转检测
4. 无预测记录和事后验证
5. 只看最近一天，不看一周格局 — 6/29复盘：3.00多次测试不破应看涨

V2 改进:
1. 优先使用腾讯实时行情
2. 平局（-5~+5分）输出"震荡"，不强行选边
3. 日内V反检测：上午跌幅>1%时触发反弹预警+8分
4. 底部支撑检测：近5日低点与近20日低点重合(偏差<1%)→支撑确认+10分
5. 每周格局检测：5日趋势 vs 20日趋势背离时倾向均值回归
6. 每次预测自动存档，收盘后自动验证"""
...
def predict() -> dict:
    ...
    # 底部支撑检测 (10分) — 6/29复盘教训
    # 近5日最低 vs 近20日最低重合 → 支撑确认
    low5 = cl.iloc[-5:].min()
    low20 = cl.iloc[-20:].min()
    if abs(low5 - low20) / low20 < 0.01:  # 偏差<1%
        score += 10  # 支撑确认，看涨
