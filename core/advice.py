...
def _generate_details(
    ...
    details.append("")
    details.append("=== 评分细项 ===")
    details.append(f"估值因子: {bd.valuation_score:.1f} / {bd.valuation_score / 0.35 * 100:.0f}分  (权重 35%)")
    details.append(f"技术因子: {bd.technical_score:.1f} / {bd.technical_score / 0.30 * 100:.0f}分  (权重 30%)")
    details.append(f"趋势因子: {bd.trend_score:.1f} / {bd.trend_score / 0.20 * 100:.0f}分  (权重 20%)")
    details.append(f"量能因子: {bd.volume_score:.1f} / {bd.volume_score / 0.10 * 100:.0f}分  (权重 10%)")
    details.append(f"情绪因子: {bd.sentiment_score:.1f} / {bd.sentiment_score / 0.05 * 100:.0f}分  (权重 5%)")
    details.append("")
    details.append("=== 技术指标 ===")
    if analysis.ma_20:
